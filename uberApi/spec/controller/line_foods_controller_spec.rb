...
RSpec.describe Api::V1::LineFoodsController, type: :controller do
  ...
  describe "indexの正常確認" do
    ...
    it "indexの仮注文レスポンスデータ取得" do
      ...
      expect(result['amount']).to eq 302
    end
  end
  ...
  describe "replaceの正常確認" do
    ...
    it "replace処理の正常ステータスコード" do
      food_id = 3
      count = 2
      
      put :replace, params: { food_id: food_id , count: count }
      expect(response).to have_http_status '201' 
    end

    it "replace処理の更新確認" do
      food_id = 3
      count = 2
      
      put :replace, params: { food_id: food_id , count: count }

      results = LineFood.all

      replaceResponse = JSON.parse(response.body)['line_food']

      # 論理削除した仮注文データ
      expect(results[0].active).to eq false 
      expect(results[0].count).to eq 1 
      expect(results[0].restaurant.name).to eq 'レストラン1'
      expect(results[0].food.name).to eq 'フード0'

      # 新規で追加した仮注文データ
      expect(results[1].active).to eq true 
      expect(results[1].count).to eq 2 
      expect(results[1].restaurant.name).to eq 'レストラン2'
      expect(results[1].food.name).to eq 'フード2'

      # レスポンスデータ
      expect(replaceResponse['active']).to eq true 
      expect(replaceResponse['count']).to eq 2 
      expect(replaceResponse['restaurant_id']).to eq 2
      expect(replaceResponse['food_id']).to eq 3
    end
  end
